...
import s from './NewPersonal.module.scss';
import { twoColors } from '../../shared/utils/constData.ts';
import { QUESTIONS } from '../../shared/constants/questions.ts';
import { lazy, Suspense, useState } from 'react';
import { UserName, UserAge, UserLocation, UserStack, UserProject, UserLanguage, UserTelegram, UserAbout, UserEducation, UserExperience, UserAvatar, UserDirection, UserGithub } from '../../features/ResumeForm/components/index.ts';
import { useFormContext } from 'react-hook-form';
import { validationRulesForm } from '../../features/ResumeForm/model/validationRulesForm.ts';
import { Loader } from '../../shared/ui/Loader/Loader.tsx';

const NewPersonal = () => {
  const PreviewResume = lazy(() => import('../../features/PreviewResume/PreviewResume.tsx'));
  ...
  const renderStepComponent = (step: number) => {
    const stepComponents = [
      ...
      <UserEducation step={step} setStep={setStep} />,
      ...
      <UserTelegram control={control} />,
      <UserGithub control={control} />,
      <UserAvatar control={control} />,
      <UserAbout control={control} />
    ];

    return <Suspense fallback={<Loader />}>{stepComponents[step]}</Suspense>
  }
